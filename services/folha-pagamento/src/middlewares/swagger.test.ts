 ...
 *    casam com `/folhas` ou `/holerite/` com:
...
async function buildAppWithSwagger(): Promise<FastifyInstance> {
  ...
  // Rotas /folhas + /holerite — alvos do enrichment do transformObject
  app.get(
    '/folhas/teste',
    { schema: { tags: ['Folhas'], response: { 200: z.object({ ok: z.boolean() }) } } },
    async () => ({ ok: true }),
  )
  app.get(
    '/funcionarios/FUN1/holerite/2026-05',
    { schema: { tags: ['Holerite'], response: { 200: z.object({ ok: z.boolean() }) } } },
    async () => ({ ok: true }),
  )
  // Rota fora dos prefixos cobertos — não deve receber enrichment
  app.get(
    '/outro',
...
describe('swagger middleware', () => {
  ...
  it('SWAGGER_ENABLED=true → rotas /folhas* recebem responses padrão + security bearerAuth', async () => {
    vi.stubEnv('SWAGGER_ENABLED', 'true')
    app = await buildAppWithSwagger()
    const res = await app.inject({ method: 'GET', url: '/docs/json' })
    const spec = JSON.parse(res.payload) as SwaggerSpec
    const op = spec.paths['/folhas/teste']?.['get']
  ...
  it('SWAGGER_ENABLED=true → rotas /holerite/* também recebem enrichment', async () => {
    vi.stubEnv('SWAGGER_ENABLED', 'true')
    app = await buildAppWithSwagger()
    const res = await app.inject({ method: 'GET', url: '/docs/json' })
    const spec = JSON.parse(res.payload) as SwaggerSpec
    const op = spec.paths['/funcionarios/FUN1/holerite/2026-05']?.['get']
    expect(op).toBeDefined()
    expect(op!.responses).toHaveProperty('401')
    expect(op!.security).toEqual([{ bearerAuth: [] }])
  })
  ...
  it('SWAGGER_ENABLED=true → rota fora dos prefixos NÃO recebe enrichment', async () => {
    ...
    expect(op).toBeDefined()
    // Sem security e sem responses adicionadas pelo transformObject
