 ...
 *
 * Configuração:
 *   - KAFKA_ENABLED=false → cliente é no-op, services seguem funcionando
 *     sem Kafka (LogEventPublisher cobre o publish, consumers não rodam).
 *   - KAFKA_ENABLED=true → conecta via KAFKA_BROKERS, expõe producer e
 *     consumer factory.
 *
 * Lifecycle: chamar `connectKafka()` no bootstrap do app; `disconnectKafka()`
 * no shutdown (SIGTERM).
 */
...
const CLIENT_ID = 'ms-folha-pagamento'
