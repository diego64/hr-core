...
describe('cors middleware', () => {
  ...
  it('CORS_ORIGINS="" → não registra @fastify/cors, request sem header Origin passa', async () => {
    ...
    // sem cors registrado → preflight não tem ACAO header
    expect(res.headers['access-control-allow-origin']).toBeUndefined()
  })

  it('CORS_ORIGINS="*" → reflete origin recebida (@fastify/cors com origin=true reflete em vez de devolver "*" literal)', async () => {
    vi.stubEnv('CORS_ORIGINS', '*')
    app = await buildAppWithCors()
    const res = await app.inject({
      method: 'OPTIONS',
