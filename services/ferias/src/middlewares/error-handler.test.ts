...
describe('error-handler', () => {
  ...
  it('DomainError → status do erro com type=code', async () => {
    ...
    expect(body.detail).toBe('detalhe específico')
  })
  ...
  it('Erro genérico statusCode 500 → type=internal, detail mascarado, log.error', async () => {
    await buildApp((a) => {
      a.get('/internal', () => {
        const err = new Error('Erro real interno (não deve vazar)') as Error & {
          statusCode?: number
        }
        err.statusCode = 500
        throw err
  ...
  it('Erro sem name → fallback "Error"', async () => {
    await buildApp((a) => {
      a.get('/anon', () => {
  ...
  it('ZodError multi-campo → fieldErrors contém todos os campos', async () => {
    await buildApp((a) => {
      a.get('/zod-multi', () => {
