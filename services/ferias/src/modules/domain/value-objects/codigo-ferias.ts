/**
 * Código legível da solicitação de férias.
 *
 * Formato: `FER` + sequencial com 6 dígitos (zero-padded). Cresce naturalmente
 ...
 */
export function gerarCodigoFerias(sequencia: number): string {
  const numero = String(sequencia)
  const zeros = Math.max(0, 6 - numero.length)
  return `FER${'0'.repeat(zeros)}${numero}`
}
